import random
from ANJALI.database import get_served_chats
...
@ANJALI.on_message(filters.command(SHAYRI_COMMAND))
            ...
            [
                [
                    InlineKeyboardButton(
                        "🔺𝐇σиɛʏ🔻", url=f"https://t.me/HeartBeat_Fam"
                    ),
                    InlineKeyboardButton(
                        "🔺𝐏ɛρρɛя🔻", url=f"https://t.me/HeartBeat_Offi"
                    ),
                ]
            ]
        ),
...
scheduler.add_job(send_good_night, trigger="cron", hour=23, minute=0)
...
scheduler.add_job(restart_ANJALI, trigger="cron", hour=12, minute=0)
scheduler.add_job(restart_ANJALI, trigger="cron", hour=15, minute=0)
scheduler.add_job(restart_ANJALI, trigger="cron", hour=18, minute=0)
scheduler.add_job(restart_ANJALI, trigger="cron", hour=21, minute=0)
scheduler.start()
